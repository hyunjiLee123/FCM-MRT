...
parser.add_argument('--data_path', type=str, default='/home/hyunji/Documents/FCM-MRT/data', required=False, help='Path to CIFAR and CIFAR-C directories')
...
def BlockFreqTune(orig, preprocess):
    tensorize, normalize = preprocess['tensorize'], preprocess['normalize']
    transform = FreqTune_transform.BlockFreqTune()
    transform_img = transform(orig)
    img = tensorize(transform_img)
    return normalize(img)
...
class BlockFreqTuneDataset(torch.utils.data.Dataset):
  def __init__(self, dataset, preprocess):
      self.dataset = dataset
      self.preprocess = preprocess
  ...
  def __getitem__(self, i):
    x, y = self.dataset[i]
    return BlockFreqTune(x, self.preprocess), y         ###################
